...
int main(int argc, char **argv) {
    ros::init(argc, argv, "item_deliver_node");

    // Get hovering location in parameters
    geometry_msgs::Point deliver_target;
    ros::NodeHandle param_nh("~");
    deliver_target.x = param_nh.param("deliver_x", 0.);
    deliver_target.y = param_nh.param("deliver_y", 0.);
    deliver_target.z = param_nh.param("deliver_z", 1.);
    ...
    ros::Publisher catapult_pub = nh.advertise<std_msgs::Empty>("servo/all/open", 1);
    ...
    while (ros::ok()) {
        ...
        switch (fsm_state) {
            ...
            case 2:  // Takeoff state
                if (current_pose.pose.position.z > deliver_target.z) {
                ...
                break;
            case 3:  // Move state
                if (getLengthBetweenPoints(deliver_target, current_pose.pose.position) > 0.3) {
                    last_srv_request = ros::Time::now();
                }
                if (ros::Time::now() - last_srv_request > ros::Duration(1.5)) {
                    fsm_state = 4;  // goto deliver state
                } else {  // PID control
                    twist.twist = get_pid_vel(deliver_target);
                }
                break;
            case 4:  // Deliver state
                {
            ...
            case 6:  // Move back state
                if (getLengthBetweenPoints(current_pose.pose.position, .0, .0, deliver_target.z) > 0.3) {
                    last_srv_request = ros::Time::now();
                }
                if (ros::Time::now() - last_srv_request > ros::Duration(1.5)) {
                    ...
                    origin_point.z = deliver_target.z;
